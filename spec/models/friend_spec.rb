...
RSpec.describe Friend, type: :model do
  ...
  subject(:friend) { Friend.new(user: user, name: "Alice") }
  ...
  describe "last_catchup" do
    it "returns the most recent catchup record" do
      ...
      last = friend.catchups.create!(happened_at: Time.now - 100)
      expect(friend.reload.last_catchup).to eq(last)

      new_last = friend.catchups.create!(happened_at: Time.now)
